...
import com.muzi.constants.RedisConstants;
import com.muzi.entity.LoginForm;
import com.muzi.entity.User;
import com.muzi.entity.dto.UserDTO;
import com.muzi.exception.ServiceException;
import com.muzi.mapper.UserMapper;
...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    public UserDTO login(LoginForm loginForm) {
        // 必填字段校验：用户名和密码不能为空
        if (loginForm == null
                || !StringUtils.hasText(loginForm.getUsername())
                || !StringUtils.hasText(loginForm.getPassword())) {
            throw new ServiceException(Constants.CODE_400, "用户名和密码不能为空");
        }
    ...
    public User register(LoginForm loginForm) {
        // 必填字段校验：用户名、密码、手机号不能为空（邮箱可选）
        if (loginForm == null
                || !StringUtils.hasText(loginForm.getUsername())
                || !StringUtils.hasText(loginForm.getPassword())
                || !StringUtils.hasText(loginForm.getPhone())) {
            throw new ServiceException(Constants.CODE_400, "用户名、密码和手机号不能为空");
        }
