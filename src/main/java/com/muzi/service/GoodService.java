...
import com.muzi.entity.GoodStandard;
import com.muzi.entity.dto.GoodDTO;
import com.muzi.exception.ServiceException;
import com.muzi.mapper.GoodMapper;
...
import static com.muzi.constants.RedisConstants.GOOD_TOKEN_KEY;
import static com.muzi.constants.RedisConstants.GOOD_TOKEN_TTL;
    ...
    public Long saveOrUpdateGood(Good good) {
        ...
        try {
            if(good.getId()==null){
                ...
                // 确保 ID 已设置（MyBatis 会自动回填）
                if(good.getId() == null) {
                    throw new ServiceException(Constants.CODE_500, "保存商品失败：未获取到商品ID，请检查数据库配置");
                }
            }else{
                saveOrUpdate(good);
            ...
            e.printStackTrace();
            throw new ServiceException(Constants.CODE_500, "保存商品失败：" + e.getMessage());
        }
    }
